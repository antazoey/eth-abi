...
CORRECT_TUPLE_ENCODINGS = (
    CORRECT_STATIC_TUPLE_ENCODINGS + CORRECT_DYNAMIC_TUPLE_ENCODINGS
)
...
CORRECT_DYNAMIC_SINGLE_ENCODINGS = [
    ...
    (
        "bytes[]",
        [b"\xde\xad\xbe\xef"],
        words("1", "20", "4", "deadbeef>0"),
        b"\xde\xad\xbe\xef",
    ),
]

CORRECT_STATIC_ENCODINGS = (
    CORRECT_STATIC_TUPLE_ENCODINGS + CORRECT_STATIC_SINGLE_ENCODINGS
)
CORRECT_DYNAMIC_ENCODINGS = (
    CORRECT_DYNAMIC_TUPLE_ENCODINGS + CORRECT_DYNAMIC_SINGLE_ENCODINGS
)
...
NOT_ENCODABLE = [
    ...
    (
        "((bytes,bool),(bytes,bool))",
        ((b"david attenborough", 0), (b"boaty mcboatface", True)),
    ),
    ("(uint32)", ("6",)),
    ("int[]", 6),
    ("bool[2]", (True, 2)),
